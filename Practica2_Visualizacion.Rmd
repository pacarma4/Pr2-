...
rm(list=ls())
setwd("C:/Users/pablo/Documents/CIENCIA DATOS/AED/P2_ALUMNOS/P2_ALUMNOS/Pr2-/data")
...
grafico3 <- grafico2 + ggtitle("Grafico de dispersión de temperatura máxima frente a temperatura mínima")

ggsave("Ejercicio1.pdf",grafico3)
```
...
setwd("C:/Users/pablo/Documents/CIENCIA DATOS/AED/P2_ALUMNOS/P2_ALUMNOS/Pr2-/data")
load("weatherAUS.RData")
grafico4 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am)) +
  geom_point()
grafico4

grafico5 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am, shape = RainToday)) +
  geom_point()
grafico5

grafico6 <- ggplot(ds, aes(x=Temp9am, y=Pressure9am, color = RainToday)) +
  geom_point()
grafico6 <-grafico6 +  ggtitle("Gráfico de dispersión de presión frente a temperatura a las 9am")
grafico6

ggsave("Ejercicio2.pdf",grafico6)
```
...
irisgrafico1 <- ggplot(iris.tidy, aes(x = Species, y = Value))

irisgrafico1 <- irisgrafico1 + geom_point()

irisgrafico2 <- ggplot(iris.tidy, aes(x = Species, y = Value, color = Part)) + 
  geom_point()

irisgrafico2fil <- irisgrafico2 + facet_grid(Measure~.)
irisgrafico2fil
irisgrafico2col <- irisgrafico2 + facet_grid(~Measure)
irisgrafico2col

irisgrafico3  <- ggplot(iris.tidy, aes(x = Species, y = Value, color = Part)) + 
  geom_jitter() + facet_grid(~Measure)
irisgrafico3

ggsave("Ejercicio3.pdf",irisgrafico3)
...
rm(list=ls())
setwd("C:/Users/pablo/Documents/CIENCIA DATOS/AED/P2_ALUMNOS/P2_ALUMNOS/Pr2-/data")
load("Ejercicio4_Data.RData")

ej4graf1 <- ggplot(DatosEjer4, aes(x=Fecha, y = Ventas, group = Fabricante)) +
  geom_point(aes(color=Fabricante))

ej4graf1 <- ej4graf1 + geom_line()

ej4graf1 <- ej4graf1 + geom_line(aes(linetype = Fabricante, color = Fabricante))

ej4graf1 <- ej4graf1 +ggtitle("Ventas de Microsoft y apple en función del tiempo")
ej4graf1

ggsave("Ejercicio4.pdf", ej4graf1)
```
...
rm(list=ls())
setwd("C:/Users/pablo/Documents/CIENCIA DATOS/AED/P2_ALUMNOS/P2_ALUMNOS/Pr2-/data")
load("Experimento.RData")
library(ggplot2)

ej5gr <- ggplot(Experimento, aes(x=Dosis, y=Altura, linetype=Complemento, shape= Complemento)) + 
  geom_point() +
  geom_line()
ej5gr <- ej5gr + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura),width=0.1)
ej5gr

ej5gr2 <- ggplot(Experimento, aes(x=Dosis, y=Altura, color = Complemento)) + 
  geom_point() +
  geom_line() + 
...
ej6graf <- ggplot(ice.river.df,aes(x=time)) + geom_line(aes(y=flow.vat,color="Vat")) + geom_line(aes(y=flow.jok,color="Yok")) +
  labs(title = "Caudales de dos ríos irlandeses en función del tiempo", x ="Tiempo (años)",y ="Caudal", color = "Río")+
  scale_color_manual(values = c("Vat"="red", "Yok" = "blue"))
ej6graf
  
ggsave("Ejercicio6.pdf",ej6graf)
```
...
rm(list=ls())
setwd("C:/Users/pablo/Documents/CIENCIA DATOS/AED/P2_ALUMNOS/P2_ALUMNOS/Pr2-/data")
load("weatherAUS_distributions.RData")
library(ggplot2)

ej7gr <- ggplot(ds, aes(x = Year, y = MinTemp)) + geom_boxplot()
ej7gr

ej7gr2 <- ggplot(ds, aes(x = Year, y = MaxTemp)) + labs(title = "Distribución de temperaturas máximas por año", x = "Año", y = "Temperatura (ºC)") + 
  geom_boxplot(aes(fill = Year))
ej7gr2 <- ej7gr2 + scale_x_discrete(guide = guide_axis(angle = 45)) + theme(legend.position = "none")
ej7gr2

ggsave("Ejercicio7.pdf",ej7gr2)
```
...
Assets <- read.table("http://assets.datacamp.com/blog_assets/chol.txt", header =TRUE)

ej8gr <- ggplot(Assets, aes(x=HEIGHT)) + geom_histogram()

ej8gr <- ej8gr + labs(title = "Histograma de la variable Altura", x = "Altura (cm)", y = "Frecuencia")

ej8gr <- ggplot(Assets, aes(x=HEIGHT)) + geom_histogram(breaks=seq(155,195, by =2)) + labs(title = "Histograma de la variable Altura", x = "Altura (cm)", y = "Frecuencia")

ej8gr <- ggplot(Assets, aes(x=HEIGHT)) + 
  geom_histogram(breaks=seq(155,195, by =2),col="red", fill="blue", alpha = 0.5) +
  labs(title = "Histograma de la variable Altura", x = "Altura (cm)", y = "Frecuencia")

ej8gr <- ej8gr + geom_density(col="green",linewidth = 1,aes(y = after_stat(count * 2)))

ej8gr <- ej8gr + facet_grid(~MORT)

ej8gr

ggsave("Ejercicio8.pdf",ej8gr)
